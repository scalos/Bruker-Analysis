function cbAx = plotRGBColorbar(ax,data,cmap,isMainAx,opts)
    ...
    cbAx.YTick = tickIdx;
    ytickLbls = cell(size(tickLabels));
    for idx = 1:numel(ytickLbls)
        ytickLbls{idx} = sprintf('%.*e',opts.sigFigs-1,round(tickLabels(idx),opts.sigFigs,"significant"));
    end
    cbAx.YTickLabel = ytickLbls;
