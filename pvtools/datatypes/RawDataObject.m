    ...
    function obj = RawDataObject(varargin)
        ...
        if isempty(varargin) % -> only create empty object           
            ...
            if (length(varargin)==1 || length(varargin)==3) % -> No error!
                ...
                % try reading reco file:
                try
                    [obj.Reco, trash]=readBrukerParamFile(obj.Filespath.path_to_reco);
                    clear trash;
                catch
                    if fid_proc
                        warning('MATLAB:bruker_note', 'Reco-File can''t be read !');
                    end
                end 
...
methods (Access = private)
    function args = adaptArgsFidProc(obj, args)
        ...
        if isprop(obj, 'Reco') && isfield(obj.Reco, 'RECO_size')
            args=[args, {'RECO_size'}, {obj.Reco.RECO_size}];
        end
    end
end
