...
%----------------------------------------------------------------
    ...
    if ~isempty(RECO_size);
        paramStruct.RECO_size=RECO_size;
    end
...
function [ data ] = readFidProcFile(Acqp, paramStruct, path_to_dataFile, specified_NRs, precision, endian)
    ...
    paramStruct.NI=Acqp.NI;
    paramStruct.NR=Acqp.NR;
    if isfield(paramStruct,'RECO_size')
        paramStruct.ACQ_size=paramStruct.RECO_size;
        paramStruct.ACQ_size(1)=2*paramStruct.RECO_size(1);
    else
        % fallback
